...
class ListCardsUseCase:
    ...
    async def execute(self, household_id: UUID) -> list[CardResponse]:
        cards = await self._repo.list_by_household(household_id)
...
class GetCardUseCase:
    ...
    async def execute(self, card_id: UUID, household_id: UUID) -> CardResponse:
        card = await self._repo.get_by_id(card_id)
        if card is None:
            raise NotFoundError("Card", str(card_id))
        if card.household_id != household_id:
...
class CreateCardUseCase:
    ...
    async def execute(
        self, data: CardCreate, user_id: UUID, household_id: UUID
    ) -> CardResponse:
...
class UpdateCardUseCase:
    ...
    async def execute(
        self, card_id: UUID, data: CardUpdate, household_id: UUID
    ) -> CardResponse:
        card = await self._repo.get_by_id(card_id)
        if card is None:
            raise NotFoundError("Card", str(card_id))
...
class DeleteCardUseCase:
    ...
    async def execute(self, card_id: UUID, household_id: UUID) -> None:
        card = await self._repo.get_by_id(card_id)
        if card is None:
            raise NotFoundError("Card", str(card_id))
        if card.household_id != household_id:
